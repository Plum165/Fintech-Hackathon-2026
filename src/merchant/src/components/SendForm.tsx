...
import { ArrowLeft, ArrowRight, Loader2, Send, CheckCircle2, ExternalLink, ShieldCheck } from 'lucide-react';
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 1 && (
          <motion.form key="step1" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }}
            onSubmit={handleInitiate} className="space-y-4">
            ...
            <button type="submit" disabled={isLoading}
              className="w-full flex items-center justify-center gap-1.5 py-3 px-4 bg-indigo-600 hover:bg-indigo-700 active:scale-98 disabled:opacity-50 text-white font-semibold rounded-xl text-xs transition mt-4 cursor-pointer shadow-sm">
          ...
          <motion.div key="step2" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-5">
            ...
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl space-y-3">
              <p className="text-xs text-amber-800 font-semibold">
                This payment requires wallet approval. Click below to authorise it in your Interledger wallet, then return here to complete.
              </p>
        ...
        {step === 3 && pending && (
          <motion.div key="step3" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-5">
            ...
            <button onClick={handleExecute} disabled={isLoading}
              className="w-full flex items-center justify-center gap-1.5 py-3 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold text-xs rounded-xl transition shadow-sm cursor-pointer">
        ...
        {step === 4 && (
          <motion.div key="step4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center space-y-4">
            <Mascot state="happy" message={`Success! Sent R ${debitHuman} to $ilp.../${destination}!`} size="lg" />
