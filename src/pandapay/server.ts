...
async function start() {
  ...
  if (process.env.NODE_ENV !== 'production') {
    // Vite dev server proxies /api/* to the consolidated backend
    ...
    // Proxy /api calls to the backend in production as well
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[PandaPay Frontend] http://localhost:${PORT}  →  API: ${API_TARGET}`);
  });
}
