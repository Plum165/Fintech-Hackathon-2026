      ...
      <div className="lg:col-span-2 flex flex-col h-[480px] bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
        <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-peach-700 font-mono font-medium">Gemini 3.5 Assistant Active</p>
            </div>
          </div>
          <button
                ...
                <div
                  className={`max-w-[80%] rounded-2xl p-3.5 text-xs leading-relaxed shadow-sm ${
                    isModel
                      ...
                      : 'bg-peach-600 text-white rounded-tr-none font-medium shadow-xs'
                  }`}
                >
                  <p>{m.content}</p>
                </div>
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="bg-slate-100 border border-slate-200 text-slate-700 rounded-2xl rounded-tl-none p-3.5 text-xs flex items-center gap-2 shadow-sm">
                <Loader2 className="w-4 h-4 animate-spin text-peach-600" />
        ...
        <form onSubmit={handleSend} className="p-3 bg-white border-t border-slate-100 flex gap-2">
          <input
            ...
            className="flex-1 bg-slate-50 border border-slate-200 text-xs text-slate-800 px-4 py-3 rounded-xl focus:outline-none focus:border-peach-500 focus:ring-1 focus:ring-peach-500 font-medium"
            disabled={isLoading}
          />
          <button
            type="submit"
            disabled={!input.trim() || isLoading}
            className="p-3 bg-peach-600 hover:bg-peach-700 disabled:bg-slate-100 disabled:text-slate-400 text-white font-semibold rounded-xl transition flex items-center justify-center cursor-pointer shadow-sm"
          >
      ...
      <div className="flex flex-col gap-6">
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5 font-sans">
            <Sparkles className="w-4 h-4 text-peach-600" /> Zenny's Active Advice
          </h4>
