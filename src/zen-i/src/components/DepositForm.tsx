...
import { ArrowLeft, ArrowRight, CheckCircle2, QrCode, Sparkles, Loader2, Camera } from 'lucide-react';
...
export default function DepositForm({ currentUsername, onDepositComplete, token }: DepositFormProps) {
  ...
  const handleScanSuccess = (scannedValue: string) => {
    // Look for voucher codes like $voucher-250
    if (scannedValue.includes('voucher-')) {
      const match = scannedValue.match(/voucher-(\d+)/);
      if (match && match[1]) {
        setAmount(match[1]);
        setError('');
      }
          ...
          >
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <label className="text-xs text-slate-500 font-bold font-sans">Amount</label>
                <button
                  type="button"
                ...
                >
                  <Camera className="w-3.5 h-3.5" /> {showScanner ? 'Close Scanner' : 'Scan Voucher'}
                </button>
              </div>

              {showScanner && (
                <div className="border border-slate-200 rounded-2xl overflow-hidden mb-3">
                  <QrCodeScanner onScan={handleScanSuccess} title="Scan Prepaid Deposit Voucher QR" />
                </div>
              )}
              ...
              {quickAmounts.map((q) => (
                <button
                  ...
                  className={`py-2 text-xs font-mono font-bold rounded-xl border transition-all ${
                    amount === q
                      ? 'bg-peach-100 text-peach-800 border-peach-200 shadow-xs'
            ...
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-1.5 py-3 px-4 bg-peach-600 hover:bg-peach-700 active:scale-98 text-white font-semibold rounded-xl text-xs transition-all duration-150 mt-4 cursor-pointer shadow-sm"
            >
          ...
          >
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3 font-mono text-xs text-slate-600">
              ...
              <div className="flex justify-between pb-1">
                ...
                <span className="text-peach-700 font-bold">Open Payments (ILP)</span>
              </div>
            </div>
