        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between">
              <div className="space-y-1">
                ...
                <span className="text-xl font-bold text-peach-750 font-mono">{wallet?.currency} {totalRemainingDebt.toFixed(2)}</span>
              </div>
              <div className="bg-peach-50 p-2.5 rounded-xl text-peach-700"><CreditCard className="w-5 h-5" /></div>
            </div>
            <div className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center justify-between">
              <div className="space-y-1">
                <span className="text-[10px] uppercase font-mono tracking-wider font-bold text-slate-400 block">Active Split Plans</span>
                <span className="text-xl font-bold text-emerald-700 font-mono">{activeContracts.length} Plan{activeContracts.length === 1 ? '' : 's'}</span>
              </div>
              <div className="bg-emerald-50 p-2.5 rounded-xl text-emerald-700"><ShieldCheck className="w-5 h-5" /></div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-xs">
            <div className="px-5 py-4 border-b border-slate-150 flex items-center justify-between">
              ...
              <button onClick={loadContracts} disabled={isLoading} className="p-1.5 hover:bg-slate-50 rounded-lg text-slate-400 hover:text-slate-700 transition cursor-pointer" title="Refresh Plans">
            ...
            {contracts.length === 0 ? (
              <div className="text-center py-12 px-4 space-y-3">
                ...
                <button onClick={() => { setActiveSubTab('new-checkout'); setStep(1); }} className="px-4 py-2 text-xs font-bold text-white bg-peach-600 hover:bg-peach-700 rounded-xl transition cursor-pointer">
                            ...
                            <div className="px-5 py-4 space-y-3">
                              ...
                              <div className="space-y-2">
                                {contract.installments.map((inst) => (
                                  <div key={inst.id} className="flex items-center justify-between bg-white border border-slate-150 p-3 rounded-xl shadow-2xs">
                                    <div className="flex items-center gap-3">
                                      <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${inst.status === 'paid' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-600'}`}>
                                        {inst.installmentNumber}
                                      </div>
                                      <div>
                                        <div className="text-xs font-bold text-slate-800 font-mono">{contract.currency} {inst.amount.toFixed(2)}</div>
                                        <div className="text-[9px] text-slate-450 flex items-center gap-1.5 mt-0.5">
                                          ...
                                          {inst.status === 'paid' && inst.paidAt && (
                                            <span className="text-emerald-600 font-bold">(Paid: {new Date(inst.paidAt).toLocaleDateString()})</span>
                                          )}
                                        </div>
                                      </div>
                                    </div>
                                    <div>
                                      {inst.status === 'paid' ? (
                                        ...
                                        <button onClick={() => handleRepay(contract.id, inst.id)} disabled={isLoading} className="px-3 py-1.5 bg-peach-600 hover:bg-peach-700 disabled:bg-slate-200 disabled:text-slate-450 text-white font-bold rounded-lg text-[10px] uppercase tracking-wide transition cursor-pointer flex items-center gap-1">
                                          {isLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : 'Pay Now'}
                                        </button>
                                      )}
                                    </div>
                                  </div>
                ...
                <React.Fragment key={s}>
                  <div className="flex flex-col items-center gap-1">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border transition ${
                      isCurrent ? 'bg-peach-600 text-white border-peach-600 shadow-md ring-4 ring-peach-100'
                        : isActive ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                        : 'bg-slate-50 text-slate-400 border-slate-200'
                    }`}>
                      {s < step ? <CheckCircle className="w-3.5 h-3.5" /> : s}
                    </div>
                    <span className={`text-[9px] font-bold ${isCurrent ? 'text-peach-800' : isActive ? 'text-emerald-700' : 'text-slate-400'}`}>
                      {labels[s - 1]}
                    </span>
                  </div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  {[
                    ...
                    <button key={m.name} type="button" onClick={() => setSelectedMerchant(m.name)}
              ...
              {selectedMerchant === 'Custom' && (
                <div className="space-y-1.5">
                  ...
                  <input type="text" value={customMerchant} onChange={e => setCustomMerchant(e.target.value)}
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex rounded-xl bg-slate-50 border border-slate-200 focus-within:border-peach-500 focus-within:ring-1 focus-within:ring-peach-500 transition overflow-hidden">
                  <span className="bg-slate-100 text-slate-500 px-4 py-3 text-xs font-mono border-r border-slate-200 flex items-center font-bold">{wallet?.currency}</span>
                ...
                <div className="text-[10px] text-slate-450 font-sans mt-1">Minimum R 150.00 up to R 5,000.00.</div>
              </div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[3, 4, 6].map((num) => (
                    <button key={num} type="button" onClick={() => setTotalInstallments(num)}
                    ...
                    >
                      ...
                      <span className="text-[9px] font-mono text-slate-500 block">R {(parsedAmt / num).toFixed(2)} each</span>
                    </button>
                  ))}
                </div>
              ...
              <div className="border border-slate-150 rounded-2xl p-4 bg-slate-50 space-y-3.5 max-h-[220px] overflow-y-auto">
                <span className="text-[10px] uppercase font-mono tracking-wider font-bold text-slate-450 block sticky top-0 bg-slate-50 pb-1">Payment Schedule ({totalInstallments} Splits)</span>
                  ...
                  {Array.from({ length: totalInstallments }).map((_, idx) => {
                    ...
                    return (
                      <div key={splitIndex} className="relative">
                        <span className={`absolute -left-[19px] top-1 w-2.5 h-2.5 rounded-full ${isUpfront ? 'bg-peach-600 ring-4 ring-peach-100' : 'bg-slate-300'}`} />
                        <div>
                          ...
                          <span className="text-[10px] block mt-0.5">
                            {isUpfront ? (
                              <span className="text-emerald-600 font-medium">
                                {merchantPointer ? 'Sent via Interledger on approval' : 'Charged immediately on approval'}
                              </span>
                            ) : (
              ...
              <div className="flex items-center justify-between gap-3 pt-2">
                <button type="button" onClick={prevStep} className="w-1/3 py-3 text-slate-500 hover:text-slate-800 font-bold text-xs border border-slate-200 hover:bg-slate-50 rounded-xl transition cursor-pointer flex items-center justify-center gap-1">
                  <ArrowLeft className="w-4 h-4" /> Back
                </button>
                <button type="button" onClick={nextStep} className="flex-1 py-3 bg-peach-600 hover:bg-peach-700 text-white font-bold rounded-xl text-xs transition shadow-md flex items-center justify-center gap-1.5 cursor-pointer">
          ...
          {step === 3 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
              ...
              <div className="p-4 border border-slate-150 bg-slate-50 rounded-2xl space-y-3 font-mono text-[10px] text-slate-650 max-h-[160px] overflow-y-auto leading-relaxed">
                ...
                <p><strong>1. Purchase Splitting:</strong> The operator authorizes PandaPay to split R {parsedAmt.toFixed(2)} with {selectedMerchant === 'Custom' ? customMerchant : selectedMerchant}.</p>
                <p><strong>2. Immediate Collateral Debit:</strong> I consent to pay <strong>R {singleSplit.toFixed(2)}</strong> immediately upon final activation to cover Split 1{merchantPointer ? ' via Interledger Open Payments' : ''}.</p>
                <p><strong>3. Auto-Settle Milestones:</strong> Subsequent installments will be automatically settled from the operator's ledger balance on schedule.</p>
              ...
              <div className="space-y-1.5">
                <label className="text-xs text-slate-500 font-bold font-sans">E-Signature (Type Full Name)</label>
                <input type="text" value={electronicSignature} onChange={e => setElectronicSignature(e.target.value)}
              ...
              <div className="bg-peach-50/40 border border-peach-100 p-3.5 rounded-xl text-xs space-y-1 font-sans">
                ...
                {merchantPointer && (
                  <div className="flex justify-between text-violet-700">
                    ...
                    <span className="font-mono text-[10px]">Interledger (real ILP)</span>
                  </div>
                )}
              ...
              <div className="flex items-center justify-between gap-3 pt-2">
                <button type="button" onClick={prevStep} className="w-1/3 py-3 text-slate-500 hover:text-slate-800 font-bold text-xs border border-slate-200 hover:bg-slate-50 rounded-xl transition cursor-pointer">Back</button>
                <button type="button" onClick={handleCreateCheckout} disabled={isLoading}
                  ...
                  {isLoading ? (
                    ...
                    <><Sparkles className="w-4 h-4 text-amber-300" /> Sign & Initiate Payment</>
                  )}
                </button>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-6">
              ...
              <div className="border border-dashed border-slate-350 bg-slate-50 rounded-3xl p-5 text-left text-xs space-y-3 relative overflow-hidden font-sans">
                ...
                <div className="pt-2 border-t border-dashed border-slate-200 flex justify-between items-center text-xs">
                  <span className="text-slate-500 font-bold">Compliant Signature:</span>
                  <span className="font-mono text-slate-800 font-bold italic">{electronicSignature}</span>
                </div>
              </div>

              <div className="pt-2">
                <button type="button" onClick={resetCheckoutForm} className="px-6 py-3 bg-peach-600 hover:bg-peach-700 text-white font-bold rounded-xl text-xs transition shadow-md cursor-pointer">
