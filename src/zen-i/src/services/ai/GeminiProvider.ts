...
export class GeminiProvider implements AIProvider {
  ...
  async generateText(prompt: string, systemInstruction?: string): Promise<string> {
    ...
    if (prompt.toLowerCase().includes("budget") || prompt.toLowerCase().includes("transaction") || prompt.toLowerCase().includes("limits")) {
      return "Zen is currently meditating in the bamboo grove, but here is a wise tip: Keeping a steady buffer in your Reserve savings bucket and paying off your BNPL splits on time are the fastest ways to elevate your Interledger Confidence Score! 🐼";
    }
    return "Zen is currently offline meditating! 🐼 Remember to keep your spending within your budget limits and build a small savings buffer. Let me know if you would like me to try recalculating again in a moment!";
  }
