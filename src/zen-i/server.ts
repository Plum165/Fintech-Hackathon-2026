...
async function startServer() {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      if (isPointer) {
        ...
        // Normal email login
        if (!input.includes('@')) {
          return res.status(400).json({ error: 'Please enter a valid email address or Interledger payment pointer (e.g. $mikaeel).' });
        }
  ...
  app.post("/api/payments/send/initiate", authenticate, async (req, res) => {
    try {
      const { destination, amount, description } = req.body;
      const numAmount = parseFloat(amount);
      if (!destination || isNaN(numAmount) || numAmount <= 0) {
        return res.status(400).json({ error: 'Destination pointer and positive amount required.' });
      }
      ...
      const approvalUrl = `https://ilp.interledger-test.dev/consent/${quoteId}?client=pandapay-client&amount=${numAmount}&dest=${encodeURIComponent(destPointer)}`;
      const continueUri = `/api/payments/send/execute`;
      const continueToken = 'ctkn_' + Math.random().toString(36).substr(2, 12);
      ...
      res.json({
        ...
        quote: {
          id: quoteId,
          debitAmount: { value: (numAmount * 100).toString(), assetCode: wallet.currency, assetScale: 2 },
          receiveAmount: { value: (numAmount * 100).toString(), assetCode: wallet.currency, assetScale: 2 },
  ...
  app.post("/api/payments/send/execute", authenticate, async (req, res) => {
    try {
      const { continueUri, continueToken, quoteId, amount, destination, description } = req.body;
      const numAmount = parseFloat(amount);
      if (isNaN(numAmount) || numAmount <= 0) {
        return res.status(400).json({ error: 'Positive numeric amount required.' });
      }
      ...
      // Handle receiving wallet if it is an internal user
      const receiverUsername = destination.toLowerCase().replace('$', '').trim();
      const receiverWallet = db.wallets.find(w => w.username.toLowerCase() === receiverUsername || w.pointer.toLowerCase() === destPointer.toLowerCase());
      if (receiverWallet) {
        receiverWallet.balance += numAmount;
        
        // Boost confidence score for receiving funds!
        receiverWallet.confidenceScore = Math.min(1000, receiverWallet.confidenceScore + 10);

        const receiveTx: Transaction = {
          id: 'tx_in_' + Math.random().toString(36).substr(2, 12),
          walletId: receiverWallet.id,
          type: 'deposit',
          direction: 'in',
          amount: numAmount,
          currency: receiverWallet.currency,
          counterparty: wallet.pointer,
          reference: description || `Received from ${wallet.username}`,
          status: 'completed',
          category: 'Deposits & income',
          createdAt: new Date().toISOString()
        };
        db.transactions.unshift(receiveTx);
      }

      // Slightly increase confidence score on successful payment!
  ...
  app.post("/api/ai/chat", authenticate, async (req, res) => {
    try {
      ...
      const systemInstruction = `You are Zenny, the helpful, wise financial panda mascot of Zenny 🐼. 
